...
close(TEAMS);
#Jigz: Individual Team files
PrintTeamByNumber();

sub PrintTeamByNumber() {
#This prints individual rosters for the team; easy to send to the coordinators.
	for (my $team=1; $team <= $num_teams; $team++) {
		my @p = PlayersOnTeam($team);
		open(TEAM, ">team$team.list") || die("Unable to open file team$team file");
		print TEAM "SHIRT Size	NAME		EmailAddress\n";
    		for (my $i = 0; $i <= $#p; $i++) {
        		my $pid = $p[$i];
			my $code = F($pid, "Shirt");
    			#my $code = F($pid, "ShirtSize");
    			my $shirtSize = ShirtStringForCode($code);
			my $name = Name($pid);
			my $email = Email($pid);
			print TEAM "$shirtSize	$name		$email\n";
		}
		#Print the Email address as a comma separated value for easy copy & paste
		print TEAM "\n\nEMAIL Addresses\n";
		for (my $i = 0; $i <= $#p; $i++) {
                        my $pid = $p[$i];
                        #my $name = Name($pid);
