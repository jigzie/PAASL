...
my $fulldata_name = "Fall2015/BSunday.xls";
...
my $ratings_file ="../../Templates/BCorrected.csv";

# The number of teams we are trying to build
my $am_teams = 0;
my $dont_care_teams = 12;
my $pm_teams = 0;
...
my %group_fixups = (
   ...
   'eugene.kubovsky@gmail.com' => 'gooner'
   );
...
open(TEAMS, ">".$dir."/teaminfo.txt");
#Jigz: For New Players
open(NewPlayers, ">".$dir."/NewPlayersEmail.list");
#Jigz: Print Team#, NewPlayer Name, Jersey size, Socks
open(NewPlayersJersey, ">".$dir."/NewPlayersJersey.list");

for (my $team = 1; $team <= $num_teams; $team++) {
    PrintTeamRoster(STDOUT, $team);
    PrintTeamRoster(TEAMS, $team);

    open (localTeam, ">".$dir."/Team ${team}.csv");
    PrintIndividualTeamRoster(localTeam, $team);
    close(localTeam);
}
...
close(NewPlayers);
#Jigz: Close the handles
#PrintNewPlayersJersey(NewPlayersJersey);
close(NewPlayersJersey);
PrintTeamEmails();
close(TEAMS);
...
sub InteractiveMode() {
	...
	if ($cmd =~ m/^quit/) {
	    last;
	} elsif ($cmd =~ m/^overallstat/) {
		PrintOverallDesiredStat(STDOUT);
	} elsif ($cmd =~ m/^stats/) {
	    PrintAllTeamsInfo(STDOUT);
	} elsif ($cmd =~ m/^help/) {
	    print "Commands:\n";
	    print "  overallstats 	- Show overall FULL stats for team formation\n";
	    ...
	    print "  swap team <teamNumber> with <teamNumber>  - Swaps two teams numbers, to keep the KIT number the same (Soccer Balls) (e.g. 'swap team 3 with 5')\n";
...
sub PrintAllTeamsInfo {
    my $file = shift;
    #print "Average Distribution per team desired: Fowards:"+AverageNumForwardsOnTeams();
    for (my $team = 1; $team <= $num_teams; $team++) {
...
my $name = $dir."/paasl.output";
# my $date_str = `date +%Y%m%d.%H:%M:%S`;
# my $name2 = "output/paasl.output.$date_str";
GenerateOutput($name);
# GenerateOutput($name2);

GenerateWebTeamList($dir."/weblist.csv");
GenerateCSV($dir."/rosters.csv",
	    ...
	    0);
GenerateCSV($dir."/fulldata.csv", "team,ALL", "", 1);
...
sub AverageNumMidfieldsOnTeams {
    my $total_midfields = 0;
    for (my $team = 1; $team <= $num_teams; $team++) {
        $total_midfields += $midfields{$team};
    }
    return (1.0 * $total_midfields) / $num_teams;
}
...
#    printf "Size %3d Team $team : %2d + %d = %2d %6.4f %6.4f %6.4f avg: %4.1f\n", $total_players_placed, $N, $group_size, $new_N, $pre_score, $mult, $score, $avg_N_per_team;
    ...
    if ($g_midfields > 0) {
        # Try to balance the forwards
        my $avg_midfields = AverageNumMidfieldsOnTeams();
        if ($midfields{$team} <= $avg_midfields - 2) {
            $score *= 10;
        } elsif ($midfields{$team} <= $avg_midfields - 1) {
            $score *= 5;
        } elsif ($midfields{$team} > $avg_midfields + 1) {
            $score /= 2;
        }
...
sub UpdateStatsForTeam {
    ...
    $midfields{$team} += Midfield($pid) * $delta;
...
#Jigz: Subroutine to print NEW PLAYERS AND their Shirt Request, Shirt Request==NEW PLAYER
###### Someday add the logic of NOT in BCorrected skills list == New Player :)
sub PrintTeamEmails {
	for (my $team = 1; $team <= $num_teams; $team++) {
		open(teamEmail, ">".$dir."/Team ${team}.list");
        	my @p = PlayersOnTeam($team);
        	foreach $pid (@p) {
			my $playerEmail = Email($pid);
			print teamEmail "$playerEmail,";
		}
		#Close the handle too
		close(teamEmail);	
	}
}
sub PrintNewPlayers {
	for (my $team = 1; $team <= $num_teams; $team++) {
			...
			my $name = Name($pid);
			my $socks = SocksString($pid);
            		if ($shirt ne "None") {
                		#Jigz: Check for ordered Shirt == New Player
                		my $playerEmail = Email($pid);
                		print NewPlayers "$playerEmail,";
				print NewPlayersJersey "$team, $name, $shirt, $socks\n";
			}
		}
	}
}

#Jigz: Subroutine to print NEW PLAYERS & Jersey information per team
#sub PrintNewPlayersJersey {
#        for (my $team = 1; $team <= $num_teams; $team++) {
#                my @p = PlayersOnTeam($team);
#                foreach $pid (@p) {
#                        my $shirt = ShirtString($pid);
...
#                                #Jigz: Check for ordered Shirt == New Player
#                                my $name = Name($pid);
#				print NewPlayersJersey "$team, $name, $shirt\n";
#                        }
#                }
#        }
#}
...
sub CorrectedSkill { 
    my $pid = shift;
    my $name = Name($pid);
    #Jigz Checking if age is less than 35/30 to re-adjust ratings; new young guys change the dynamics.
    my $age = Age($pid);
    ...
    #Jigz: Check age of player; if < 35 then rating is min 3 && if < 30 then min is 2
    if ($age < 30){
        if (ReportedSkill($pid) >= 3) {
                #print "Updated Skill from $s to 2\n";
                return 2;
        }
        return min(5, max(1, ReportedSkill($pid)+$adjust));
    }
...
sub PrintOverallDesiredStat {
    #Below for each team printout... not needed but we need the loops
    #1. Loop : Teams
    #2. Loop : Players, without closing the overall hash
    #3. Divide by num of teams (static)
    #4. Calculate ideal distribution of age
    my %levels = ();
	for ($sk = 1; $sk <= 5; $sk++) {
		$levels{$sk} = 0;
	}
	...
	for (my $team = 1; $team <= $num_teams; $team++) {
		#Stats to collect/derive:
		#1. Desired Level distribution [Level 1, Level 2..., Level 5]
		#2. Desired age distributions [<35, >35, <45, <55...., >65]
		#3. Desired Positions [Defense, MidField, Forward, Goalie]
		# Above information helps to quickly move players around the teams
		
		my @p = PlayersOnTeam($team);
		#Go through each player on team
		for (my $i = 0; $i <= $#p; $i++) {
			 my $pid = $p[$i];
			 ...
			 $midfields += Midfield($pid);
			 $games += GamesPlayed($pid);
			 $avg_age += $a;
		}
		$total_players = $total_players + $#p+1;
	}#This is ALL Teams calculated as ONE team
	printf "Total Teams: %d, Players: %d, Avg. Level: %0.2f, Avg Age: %0.2f\n",$num_teams, $total_players, ($lvl_sum/$total_players), ($avg_age/$total_players); 

	printf "Age Distribution: [<30: %0.1f, <35: %0.1f, <45: %0.1f, <55: %0.1f, >55: %0.1f, >65: %0.1f]\n",$under_30/$num_teams, $under_35/$num_teams, $under_45/$num_teams, $under_55/$num_teams, $over_55/$num_teams, $over_65/$num_teams;

		
	printf "Levels Average L1: %0.2f  L2: %0.2f  L3: %0.2f  L4: %0.2f  L5: %0.2f\n", $levels{1}/$num_teams, $levels{2}/$num_teams,$levels{3}/$num_teams,$levels{4}/$num_teams,$levels{5}/$num_teams;
}
...
sub PrintTeamInfo {
    ...
    printf $file "] Games: %4.1f Age: %4.1f <30: %d <35: %d <45: %d <55: %d >55: %d >65: %d Goalies:%3.1f  D:%d M:%d F:%d\n", (1.0 * $games) / $player_count{$team}, (1.0 * $total_age{$team}) / $player_count{$team}, $under_30, $under_35, $under_45, $under_55, $over_55, $over_65, $goalies{$team}, $defenders{$team}, $midfields, $forwards;
}
...
sub PrintIndividualTeamRoster {
    my $file = shift;
    my $team = shift;

    printf $file "Name                             ShirtSize    Email           Position\n";
    my @p = PlayersOnTeam($team);
    @p = sort {
      my $cs_a = CorrectedSkill($a);
      my $cs_b = CorrectedSkill($b);
    ...
    for (my $i = 0; $i <= $#p; $i++) {
        my $pid = $p[$i];
        my $sk = CorrectedSkill($pid);
        printf $file "%-25s      %s      %s     %s\n", Name($pid), ShirtString($pid), Email($pid), Position($pid);
    }
    print $file "\n";
}
