...
# c:\oracle\product\11.2.0\dbhome_1\perl\bin\perl.exe ./make-teams.pl paasl.pin
# C:\Oracle\DB11g\product\11.2.0\dbhome_1\perl\bin\perl.exe ./make-teams.pl paasl.pin
...
# Dumps (HTM Files) for the Registered Players can be obtained via something like:
#  #  http://www.paasl.us/Admin/RegistrationData.cfm (username SushiAdmin, password SushiLover1) 
# (Actual id number varies from season to season).
my $fulldata_name = "110612-Summer_11_B_corrected.htm";
...
my $ratings_file = "BPlayerRatings-corrected.csv";

# The number of teams we are trying to build
my $am_teams = 8;
my $dont_care_teams = 0;
my $pm_teams = 0;

my $players_per_team = 22;
...
# 'correct_group').
my %group_fixups = (
    'xNickola Milkovski' => 'cska1234',
    'xGary Kirkham' => 'wahoos',
    'xlarrysmith7@comcast.net' => 'wahoos',
 );
...
my %coordinators_to_avoid = (
    ...
    # Other misc. reasons
    'rgardner1@gmail.com' => 1,  # Phil de Monet requested that we give Rob a break and pick Phil to be a coordinator instead
...
my @players_by_skill = GetPlayersBySkill($players_to_place);
...
sub InteractiveMode() {
	...
	if ($cmd =~ m/^quit/) {
		...
		PrintWaitingList(STDOUT);
		PrintTeamRoster(STDOUT, $new_team);
		print "new: "; PrintTeamInfo(STDOUT, $new_team);
		...
		AddPlayerToTeam($pid, $new_team);
		PrintTeamRoster(STDOUT, $new_team);
		print "old: "; PrintTeamInfo(STDOUT, $old_team);
		print "new: "; PrintTeamInfo(STDOUT, $new_team);
	    }
	    VerifyGroups(0);
	    ...
	    my $n1 = $1;
	    my $n2 = $3;
	    my $pid1 = FindPlayerByName($n1);
	    my $pid2 = FindPlayerByName($n2);
	    if (!defined($pid1)) {
		print "Player not found: $n1\n";
	    } elsif (!defined($pid2)) {
		print "Player not found: $n2\n";
	    } else {
		my $old_t1 = $pid_to_team{$pid1};
		my $old_t2 = $pid_to_team{$pid2};
...
sub GetPlayersBySkill {
    ...
    @player_list = sort({ 
#	my $skill_a = CorrectedSkill($a) - Goalie($a) * 100;
#	my $skill_b = CorrectedSkill($b) - Goalie($b) * 100;
	my $skill_a = CorrectedSkill($a) - Goalie($a) * 0;
	my $skill_b = CorrectedSkill($b) - Goalie($b) * 0;
	if ($skill_a != $skill_b) {
...
sub IsPaloAltoResident {
    ...
    if ($c =~ m/(palo alto|los altos hills|stanford)/) {
	return 1;
    } else {
	return 0;
    }
...
sub ShirtStringForCode {
    my $c = shift;
    if ($c == 0) { return "None"; }
    elsif ($c == 2) { return "Small"; }
    elsif ($c == 3) { return "Medium"; }
    elsif ($c == 4) { return "Large"; }
    elsif ($c == 5) { return "XL"; }
    elsif ($c == 6) { return "XXL"; }
...
sub PrintTeamInfo {
    ...
    my $team = shift;
    my @p = PlayersOnTeam($team);
    printf $file "Team %2d %-3s : %2d players; Lvls: ", $team, TeamType($team), $#p+1;
    my %levels = ();
    for ($sk = 1; $sk <= 5; $sk++) {
	$levels{$sk} = 0;
    }
    ...
    for ($sk = 1; $sk <= 5; $sk++) {
	printf $file "%d ", $levels{$sk};
    }
    printf $file " Games: %4.1f Avg age: %4.1f <30: %d <35: %d >45: %d Goalies: %3.1f  D: %3.1f F: %d\n", (1.0 * $games) / $player_count{$team}, (1.0 * $total_age{$team}) / $player_count{$team}, $under_30, $under_35, $over_45, $goalies{$team}, $defenders{$team}, $forwards;
}
